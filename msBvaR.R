...
xm <- msbvar(df, p=1, h=3,
             ...
             initialize.opt=NULL)
# varobj=xm
# A0=(xm$init.model$mean.S)


posterior.impulses <- mc.irf.BSVAR(xm, nsteps=10, draws=5000)





irf.msbvar<-irf.MSBVAR(xm, nsteps=10)

#check
plot(irf.msbvar)


"irf.MSBVAR" <- function(varobj=xm, nsteps, A0=chol(varobj$init.model$mean.S)){
  output <- .Call("irf.var.cpp", as.double(varobj$init.model$ar.coefs),
                  as.integer(dim(varobj$init.model$ar.coefs)),
                  as.integer(nsteps),
                  A0)
  
  attr(output, "class") <- c("irf", "irf.MSBVAR")
